...
function Entrance() {
    ...
    useEffect(() => {
        ...
        Config.init({
            app: 54140536,
            redirectUrl: 'https://from-sochi.github.io/STAR-WARS/', // ← УБЕДИТЕСЬ ЧТО ЭТОТ URL ТОЧНО СОВПАДАЕТ С НАСТРОЙКАМИ VK
            responseMode: ConfigResponseMode.Callback,
            source: ConfigSource.LOWCODE,
            scope: 'email',
        });
        ...
        .on(OAuthListInternalEvents.LOGIN_SUCCESS, (payload: LoginSuccessPayload) => {
            ...
            Auth.exchangeCode(payload.code, payload.device_id)
                .then((data: AuthSuccessData) => {
                    ...
                    // ДЕЛАЕМ РЕДИРЕКТ НА DASHBOARD ← ДОБАВЬТЕ ЭТО
                    navigate('/dashboard/main'); // или ваш путь
    ...
    }, [navigate]); // ← ДОБАВЬТЕ navigate В ЗАВИСИМОСТИ
    ...
    return (
        <div style={{ padding: '20px', margin: 'auto', minWidth: '300px', maxWidth: '600px' }}>
            ...
            <div style={{ marginTop: '20px', fontSize: '12px', color: '#666' }}>
                <p>App ID: 54140536</p>
                <p>Redirect URL: https://from-sochi.github.io/STAR-WARS/</p> {/* ← ОБНОВИТЕ ТЕКСТ */}
            </div>
        </div>
    );
}
